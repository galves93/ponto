...
class ListDetalhadoVO {
  ...
  factory ListDetalhadoVO.fromJson(Map<String, dynamic> json) {
    ...
    vDetalhado =
        vDetalhado.map((i) => DetalhadoVO.fromJson(i)).toList();
...
class DetalhadoJsonVO {
  ...
  Map<DateTime, List<dynamic>> toJson(List<DetalhadoDiasVO> list) {
    ...
    for (var node in list) {
      json.putIfAbsent(
          node.dia, () => node.listaFornecedor.map((f) => f.toJson()).toList());
          
      if (DateFormat("yyyy-MM-dd").format(node.dia) == DateFormat("yyyy-MM-dd").format(DateTime.now()) && gProviderNotifier.eventoAF.selectedEvents.isEmpty){
        gProviderNotifier.eventoAF.selectedEvents = node.listaFornecedor.map((f) => f.toJson()).toList();
      }
    }
    return json;
  }
